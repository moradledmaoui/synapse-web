...
interface Portfolio {
  ...
  drawdown_pct: number; win_rate: number; total_trades: number;
  open_positions: number; regime: string; regime_data: any;
}
interface Position {
...
function SlTpBar({ entry, current, sl, tp, score }: {
  ...
  const color = pct < 20 ? "#ef4444" : pct > 80 ? "#16a34a" : "#ca8a04";
  return (
    <div className="mt-3">
      <div className="relative h-[3px] rounded-full bg-gray-100">
        <div className="absolute h-full rounded-full" style={{ width: pct + "%", background: "linear-gradient(90deg, transparent, " + color + "55)" }} />
        <div className="absolute top-1/2 -translate-y-1/2 -translate-x-1/2" style={{ left: pct + "%" }}>
          <div className="w-[10px] h-[10px] rounded-full border-2 border-white shadow-sm" style={{ background: color }} />
        </div>
      </div>
      <div className="flex justify-between mt-1.5 text-[10px] font-mono">
        <span className="text-red-400">SL {fmtPrice(sl)}</span>
        <span className="text-gray-400">score {Math.round(score)}/100</span>
...
function PositionCard({ pos }: { pos: Position }) {
  ...
  const strat = STRAT[pos.strategy] || pos.strategy;
  ...
  const tvUrl = "https://www.tradingview.com/chart/?symbol=BINANCE:" + pos.symbol;

  return (
    <div className={"border rounded-xl overflow-hidden transition-colors " + (open ? "border-gray-300 bg-gray-50" : "border-gray-200 bg-white hover:border-gray-300")}>
      <button className="w-full px-4 pt-3.5 pb-3 text-left" onClick={() => setOpen(!open)}>
        <div className="flex items-center gap-3">
          <div className="relative w-9 h-9 rounded-full bg-gray-100 flex-shrink-0 flex items-center justify-center overflow-hidden">
            {pos.logo_url ? (
              <img src={pos.logo_url} alt="" className="w-full h-full object-cover" onError={(e) => { e.currentTarget.style.display = "none"; }} />
            ) : (
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="text-[11px] text-gray-400 mt-0.5 font-mono">{strat} {dur}h ${fmt(pos.size_usdt, 0)}</div>
          </div>
          <div className="text-right">
            <div className={"text-sm font-mono font-medium " + (up ? "text-green-600" : "text-red-500")}>{up ? "+" : ""}{fmt(pos.pnl_usdt)} USDT</div>
            <div className={"text-[11px] font-mono " + (up ? "text-green-500" : "text-red-400")}>{up ? "+" : ""}{fmt(pos.pnl_pct)}%</div>
          </div>
        ...
        <SlTpBar entry={pos.entry_price} current={pos.current_price} sl={pos.stop_loss} tp={pos.take_profit} score={score} />
      </button>
      {open && (
        <div className="px-4 pb-4 pt-3 border-t border-gray-100 bg-gray-50 space-y-3">
          <div className="grid grid-cols-3 gap-2">
            {[
              { label: "Entree", value: "$" + fmtPrice(pos.entry_price) },
              { label: "Actuel", value: "$" + fmtPrice(pos.current_price), color: up ? "text-green-600" : "text-red-500" },
              { label: "Regime", value: pos.regime_at_open || "--" },
          ...
          <div className="flex items-center justify-between">
            <a href={tvUrl} target="_blank" rel="noopener noreferrer" className="text-[10px] text-gray-400 underline hover:text-gray-600 font-mono">
              Voir sur TradingView
            </a>
            <button onClick={async () => { if (confirm("Fermer " + pos.symbol + " ?")) { await fetch(API + "/api/mcp/close_position?symbol=" + pos.symbol, { method: "POST" }); } }} className="px-3 py-1.5 rounded-lg border border-red-200 text-red-500 text-xs hover:bg-red-50 transition-colors">
              Fermer
            </button>
          </div>
        </div>
...
function DexCard({ pos }: { pos: any }) {
  ...
  const progPct = entry > 0 && target > entry ? Math.min(100, ((current - entry) / (target - entry)) * 100) : 0;
    ...
    <div className="border border-gray-200 bg-white rounded-xl overflow-hidden hover:border-gray-300 transition-colors">
      <button className="w-full px-4 pt-3.5 pb-3 text-left" onClick={() => setOpen(!open)}>
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="text-[11px] text-gray-400 mt-0.5 font-mono">${fmt(pos.size_usdt, 0)} target x2</div>
          </div>
          <div className="text-right">
            <div className="text-sm font-mono font-medium text-yellow-600">{x.toFixed(2)}x</div>
        ...
        <div className="mt-3">
          <div className="h-[3px] bg-gray-100 rounded-full overflow-hidden">
            <div className="h-full rounded-full bg-gradient-to-r from-yellow-300 to-green-400" style={{ width: Math.max(2, progPct) + "%" }} />
          </div>
          <div className="flex justify-between mt-1.5 text-[10px] font-mono text-gray-400">
            <span>${fmtPrice(entry)}</span>
            <span>{Math.round(progPct)}% vers x2</span>
      ...
      {open && (
        <div className="px-4 pb-3 pt-3 border-t border-gray-100 bg-gray-50">
          <p className="text-[11px] text-gray-500 font-mono">Position simulee. A x2 : recupere ${fmt(pos.size_usdt / 2, 0)} USDT et laisse courir.</p>
        </div>
      )}
    </div>
  );
    ...
    <div className="min-h-screen bg-gray-50 text-gray-900 pb-24">
      ...
      <div className="px-6 pt-5 space-y-5">
        <div>
          ...
          <div className="flex items-center gap-2 mt-1 flex-wrap text-[12px] font-mono text-gray-500">
            <span className={pnlUp ? "text-green-500" : "text-red-400"}>{pnlUp ? "+" : ""}{fmt(portfolio.pnl_pct)}%</span>
            <span className="text-gray-300">·</span>
            <span>${(portfolio.total_value || 0).toLocaleString("fr-FR", { maximumFractionDigits: 0 })} total</span>
            <span className="text-gray-300">·</span>
            <span>${(portfolio.capital || 0).toLocaleString("fr-FR", { maximumFractionDigits: 0 })} libre</span>
            {(portfolio.drawdown_pct || 0) > 0 && (<><span className="text-gray-300">·</span><span className="text-orange-400">DD {fmt(portfolio.drawdown_pct)}%</span></>)}
          </div>
        </div>

        {(fg != null || dom != null) && (
          <div className="border border-gray-200 rounded-xl p-4 bg-white">
            <div className="text-[10px] text-gray-400 uppercase tracking-widest mb-3">Contexte marche</div>
            <div className="flex items-center gap-6">
              {fg != null && (<div><div className={"text-xl font-mono font-semibold " + (fg < 25 ? "text-red-500" : fg < 45 ? "text-orange-400" : fg > 75 ? "text-green-500" : "text-yellow-500")}>{fg}</div><div className="text-[10px] text-gray-400 mt-0.5">Fear & Greed</div></div>)}
              {dom != null && (<div><div className="text-xl font-mono font-semibold text-gray-900">{fmt(dom, 1)}%</div><div className="text-[10px] text-gray-400 mt-0.5">BTC Dom</div></div>)}
              {adx != null && (<div><div className="text-xl font-mono font-semibold text-gray-900">{fmt(adx, 0)}</div><div className="text-[10px] text-gray-400 mt-0.5">ADX</div></div>)}
              {squeeze != null && (<div><div className={"text-lg font-mono font-semibold " + (squeeze ? "text-yellow-500" : "text-gray-300")}>{squeeze ? "SQ" : "--"}</div><div className="text-[10px] text-gray-400 mt-0.5">Squeeze</div></div>)}
            </div>
            {squeeze && <p className="text-[11px] text-yellow-500 mt-3 pt-3 border-t border-gray-100 font-mono">Squeeze actif - explosion imminente</p>}
            {fg != null && fg < 25 && <p className="text-[11px] text-blue-400 mt-3 pt-3 border-t border-gray-100 font-mono">Peur extreme ({fg}) - moment d accumulation</p>}
          </div>
        )}
        ...
        <div>
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="flex items-center gap-3">
              {(posData?.total_pnl || 0) !== 0 && (<span className={"text-sm font-mono " + ((posData?.total_pnl || 0) >= 0 ? "text-green-600" : "text-red-500")}>{(posData?.total_pnl || 0) >= 0 ? "+" : ""}{fmt(posData?.total_pnl)} USDT</span>)}
          ...
          {positions.length === 0 ? (
            <div className="border border-gray-200 rounded-xl p-8 text-center bg-white">
              ...
              <p className="text-gray-300 text-xs mt-1 font-mono">{squeeze ? "Signal imminent" : "En attente du prochain signal"}</p>
            </div>
          ) : (
            <div className="space-y-3">{positions.map((pos) => <PositionCard key={pos.id} pos={pos} />)}</div>
          )}
        </div>
